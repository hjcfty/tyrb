...
const DEFAULT_TYPE = '演唱会#音乐#歌曲#老电影#沙雕动漫#短剧#鬼畜#搞笑#相声#小品#戏曲#音乐#MV#舞曲#舞蹈#纪录片#健身#帕梅拉#武术#太极拳#广场舞#体育#球星#世界杯#UP主#小姐姐#女优#美食#食谱#荒野求生#旅游#风景#游戏#解说#演讲#考公考试#平面设计#软件教学'; // 分类兜底
...
async function init(config) {
    ...
    if (cookieSource) {
        if (cookieSource.startsWith('http://') || cookieSource.startsWith('https://')) {
            try {
                let resp = await req(cookieSource, {
                    headers: {
...
async function detail(ids) {
    ...
    if (!idStr || idStr.indexOf('@') === -1) return JSON.stringify({
        list: []
    });
    ...
    try {
        ...
        try {
            ...
            let statResp = await req(statUrl, {
        ...
        let director = '🆙 ' + '[a=cr:' + JSON.stringify({
            id: upName + '_clicklink',
            name: upName
        }) + '/]' + upName + '[/a]';
        ...
        try {
            ...
            let relResp = await req(relatedUrl, {
                headers: getHeaders(true)
            });
...
async function play(flag, id) {
    if (!id || id.indexOf('+') === -1) return JSON.stringify({
        parse: 0,
        url: ''
    });
    let parts = id.split('+'),
    ...
    try {
        ...
        let resp = await req(dashUrl, {
            headers: getHeaders(true)
        });
        let data = JSON.parse(resp.content);
        if (!data.data || !data.data.dash) return JSON.stringify({
            parse: 0,
            url: ''
        });
        ...
        if (formats.length === 0) {
            ...
            formats = aq.map((q, i) => ({
                quality: q,
                display_desc: ad[i] || q.toString()
            }));
        }
        let qnMap = {};
        formats.forEach(f => {
            qnMap[f.quality] = f.display_desc || f.new_description || f.quality.toString();
        });
        ...
        if (urlsArray.length === 0) return JSON.stringify({
            parse: 0,
            url: ''
        });
...
async function search(keyword, quick, pg) {
    return category(keyword, pg || '1', {}, {});
}
async function searchContent(keyword, quick, page) {
    return search(keyword, quick, page);
}
